...
def test_route():
    return jsonify({'message': 'Salut din backend!'})
...
def register():
    ...
    password = data.get('password')
    role = data.get('role', 'User')  # Valoarea implicită este "User"
...
class ReviewerRequest(db.Model):
    ...
    request_type = db.Column(db.String(20), nullable=False)  # 'add', 'edit', 'delete'
    exercise_data = db.Column(db.JSON, nullable=False)  # Stores the exercise details
    status = db.Column(db.String(20), default="pending")  # 'pending', 'approved', 'rejected'
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # Correct usage
...
def get_reviewer_requests():
    ...
    try:
        pending_requests = ReviewerRequest.query.filter_by(
...
def create_request():
    ...
    # Check if there is already a pending request for this exercise and reviewer
...
def get_requests():
    ...
    for req in requests:
        ...
        if req.request_type == "edit":
            ...
            if exercise:
                exercise_data = exercise.to_dict()  # Include toate detaliile exercițiului
...
if __name__ == '__main__':
    try:
        with app.app_context():
            db.session.execute(text('SELECT 1'))  # Modificare importantă aici
